...
namespace AVMTradeReporterTests.Model
{
    public class AggregatedPoolTests
    {
        ...
        public void FromPools_AggregatesClAMMWithOldAMM()
        {
            ...
            // For (1,2): A = p1.A_real (1.1) + reverse(p2).A_real (4.5) = 5.6
            //            B = p1.B_real (2.2) + reverse(p2).B_real (3.3) = 5.5
            Assert.That(agg12.VirtualSumALevel1, Is.EqualTo(33.411611892693684301049159175M).Within(0.000000000001m));
            Assert.That(agg12.VirtualSumBLevel1, Is.EqualTo(45.523232618035869447454225347M).Within(0.000000000001m));
        ...
        public async Task GetAggregatedPoolVoteAlgo()
        {
            ...
            Assert.That(price, Is.EqualTo(6.8129419971702496226889404405m).Within(0.000000000001m));
        ...
        [Test]
        public void FromPools_AggregatesVolumes()
        {
            // Arrange
            var p1 = new AVMTradeReporter.Models.Data.Pool
            {
            ...
            // Act
            var result = AggregatedPool.FromPools(new[] { p1, p2 }).ToArray();

            // Assert
            var agg12 = result.Single(r => r.AssetIdA == 1 && r.AssetIdB == 2);
            var agg21 = result.Single(r => r.AssetIdA == 2 && r.AssetIdB == 1);

            Assert.That(agg12.Volume1H, Is.EqualTo(300));
            Assert.That(agg12.Volume24H, Is.EqualTo(3000));
            Assert.That(agg12.Volume7D, Is.EqualTo(30000));

            Assert.That(agg21.Volume1H, Is.EqualTo(300));
