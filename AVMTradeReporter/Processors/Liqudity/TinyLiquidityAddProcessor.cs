...
{
    public class TinyLiquidityAddProcessor : ILiquidityProcessor
    {
        public string AppArg { get; set; } = "6164645f6c6971756964697479";

        private readonly ILogger<TransactionProcessor> _logger;
        public TinyLiquidityAddProcessor(ILogger<TransactionProcessor> logger)
        {
            _logger = logger;
        }
            ...
            )
        {
            if (current.Tx is ApplicationNoopTransaction appCallTx)
            {
                ...
                var inner = current.Detail?.InnerTxns?.Skip(1).FirstOrDefault()?.Tx;
